...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        $link = "http://localhost/votre_dossier/reset.php?token=" . urlencode($token) . "&email=" . urlencode($email);

        // Protection XSS : échapper le lien avant de l'afficher dans le HTML
        $safeLink = htmlspecialchars($link, ENT_QUOTES, 'UTF-8');

        echo "<div style='background:#d4edda; padding:20px;'>
                <strong>Simulation d'email :</strong><br>
                Bonjour,<br>Cliquez ici pour réinitialiser : <a href='{$safeLink}'>{$safeLink}</a>
              </div>";
    } else {
